...
取りに行くので、まとめないと同じトークンを同時に使い、1本だけ成功して残りが400になる。
それを失効と誤解してログイン情報を消していたため、**1時間以上あけて開くたびにログインし直し**
になっていた。ログイン情報を捨てるのは status 400/401 のときだけ（通信エラーでは捨てない）。

**ログイン欄は `<form>` の中に置き、ログインボタンは `type="submit"`。**
iPhone / Mac のパスワード保存は submit を合図に「保存しますか？」を出すので、
div ＋ click で処理すると候補として登録されない。
...
**ペット・病院は丸ごと上書きではなく、記録1件ずつの3者マージ。** 以前は同期前に2台で同じ子の
記録をつけると、あとから同期した側の分しか残らなかった（体重や通院記録が消えた）。

- 前回同期した時点の姿を「控え」として IndexedDB `wannyan_sync_base_v1` に持つ（写真など2000文字を
  超える文字列は短い印に置き換えて入れる）。控え・この端末・サーバーを `_merge3()` で比べる
  - 足した記録は残す／控えにあって片方で消えた記録は消す／同じ項目を両方で変えたら後から変えたほう
  - 配列の要素は `id` があれば id で、無ければ中身（同じ中身が複数なら何個目か）で見分ける
  - 日付ごとのお世話（`quickCares[日付]`）のようなオブジェクトは項目ごとに再帰して合わせる
- 「変わったか」の判定は `_sig()`（キーを並べ替えてからのハッシュ）。jsonb はキー順を保たない
- 書き込みは `updated_at=eq.<読んだ時刻>` 付きの PATCH（`_writeRow`）。他の端末が先に書いていたら
  0件更新になるので、その行を読み直して合わせてから書き直す（最大3回）
- この端末で消した子は、読み直しで復活させない。消した子に別の端末が記録を足していた場合は残す
- **丸ごと上書きに戻さないこと**
